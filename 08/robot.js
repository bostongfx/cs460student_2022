...
Robot.prototype.show = function(scene) {
  /* associate a scene id for checking collison */
  this.id = sceneids
  sceneids++
...
Robot.prototype.onAnimate = function() {
  ...
  }else if (this.movement == "walk")
  {
    ...
    this.right_upperleg.quaternion.identity()
    this.left_upperleg.quaternion.slerp(
      /*setFromAxisAngle since its just X axis, PI/4 = 45 degrees in radian */
      new THREE.Quaternion().setFromAxisAngle( new THREE.Vector3( 1, 0, 0 ), Math.PI / 4 ), 0.1)
  ...
  else if (this.movement == "walk2")
  {
    ...
    this.left_upperleg.quaternion.identity()
...
Robot.prototype.onStep = function()
{
  ...
  /* Math.abs takes care of any negatives :) */
  if(Math.abs(this.root.position.z) > 500 || Math.abs(this.root.position.x) > 500)
    this.root.rotateY(180)

  for(i = 0; i < sceneids; i++)
  {
      r_a = this.all_robots[i] /* robot a (will be moved) */
      r_b = this.all_robots[i + 1] /* robot b (to check distance) */
      
      if((r_b != null || r_b != undefined) && /* ignore out of bounds */ 
       r_a.id != this.id && /* ignore this robot */
       r_a.root.position.distanceTo(r_b.root.position) < 85.0) /* check if distance is less than a certain amount */
      {
        r_a.root.rotateY(180)
      }
  }
  
  if(this.model_box.containsPoint(this.root.position))
  {
    this.root.rotateY(180)
  }
