...
class Scanner {
    ...
    public TokenInfo getNextToken() {
        ...
        while (moreWhiteSpace) {
            ...
            if (ch == '/') {
                ...
                if (ch == '/') {
                    ...
                    /*
                     * In This line we check if we read an '*' after the / to check if the beginnning of a multiline comment.
                     * The while loop will check if the end of file is reach to finish it else it will read all character till an * 
                     * follow by an / is reach if then finalized.
                     */
        ...
        switch (ch) {
            // New Partition.-------------------------------------------
            // Modulus partition. with assignment 
            case '%': 
                nextCh();
                if(ch == '='){
                    nextCh();
            ...
            // Not Operator partition.
            case '~': 
                nextCh();
                return new TokenInfo(NOT, line);
            // Or Operator partition or Or_Assignment. or Double or Operator.
            case '|': 
                nextCh();
                if(ch == '='){
                    nextCh();
    ...
    private boolean isDouble(char c){
        return (Character.toLowerCase(c) == 'd' );
    }

    // Check if the value has an Exponential expression.
    private boolean isExp(char c){
        return (Character.toLowerCase(c) == 'e');
    }
    
    // Check if the value has an Long ch.
    private boolean isLong(char c) {
        return (Character.toLowerCase(c) == 'l');
    }

    // Push The exp expression to the buffer.
    private void pushExp(StringBuffer b){
        pushBuffer(b);
        if(ch == '+' || ch == '-' ){
            pushBuffer(b);
    ...
    // pushDouble is a method who take digits and check for more digits. 
    private void pushDouble(StringBuffer b){
        pushBuffer(b);
        pushDigits(b);
        if(isExp(ch))
            pushExp(b);
    ...
    // Push value to the string Buffer and goes to the next character
    private void pushBuffer(StringBuffer b){
        b.append(ch);
        nextCh();
    }

    // Check  if is digit and push next Digit to the buffer and goes to the next Character.
    private void pushDigits(StringBuffer b){
        while (isDigit(ch)) {
            pushBuffer(b);
        }
    }
