...
enum TokenKind {
    ...
    DEFAULT("default"), DO("do"), DOUBLE("double"), FINALLY("finally"), FOR("for"), IMPLEMENTS("implements"),
    INTERFACE("interface"), LONG("long"), SWITCH("switch"), THROW("throw"), THROWS("throws"), TRY("try"),

    // Operators.
    ASSIGN("="), DEC("--"), EQUAL("=="), GT(">"), INC("++"), LAND("&&"), LE("<="), LNOT("!"),
    MINUS("-"), PLUS("+"), PLUS_ASSIGN("+="), STAR("*"), DIV("/"), REM("%"), NOT("~"), OR("|"),
    AND("&"), XOR("^"), ALSHIFT("<<"), ARSHIFT(">>"), LRSHIFT(">>>"), QUESTION("?"), COLON(":"),
    LNOT_ASSIGN("!="),DIV_ASSIGN("/="), MINUS_ASSIGN("-="), STAR_ASSIGN("*="), REM_ASSIGN("%="), 
    ARSHIFT_ASSIGN(">>="), LRSHIFT_ASSIGN(">>>="),GE(">="), ALSHIFT_ASSIGN("<<="), LT("<"), XOR_ASSIGN("^="),
    OR_ASSIGN("|="), OR_OR("||"), AND_ASSIGN("&="),
    ...
    STRING_LITERAL("<STRING_LITERAL>"), TRUE("true"), DOUBLE_LITERAL("<DOUBLE_LITERAL>"), LONG_LITERAL("<LONG_LITERAL>");
