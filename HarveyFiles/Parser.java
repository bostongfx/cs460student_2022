...
public class Parser {
    ...
    /**
     * Parses a Interface Member declaration and returns an AST for it.
     *
     * <pre>
    ...
    private JMember interfaceMemberDecl(ArrayList<String> mods) {
        ...
        if (have(VOID)) {
            ...
            interfaceDecl = new JMethodDeclaration(line, mods, name, type, params, exceptions, null);
            mustBe(SEMI);
        } else {
            type = type();
     ...
     *               | FOR forParExpression  statemetn
     * </pre>
     *
    ...
    private JStatement statement() {
        ...
        if (see(LCURLY)) {
            ...
            mustBe(SEMI);
            return new JDoStatement(line, body,exp);
        } else if (have(FOR)) {
            // Variable declaration
                // Initiallize all variable to something,
                // so the return variable can return something
            ArrayList<JStatement> forInit= null; 
            JExpression condition = null;
            ArrayList <JStatement> updates = null;
            JStatement body = null;

            // Make sure the first after consuming for the first thing is a '(' <-- LPAREN 
            // for structure (forInit; condition; updates);
            mustBe(LPAREN);
            // if it doesnt read SEMI in the next character read the initial of the for.
            if(!see(SEMI))
                forInit = forInit();
            mustBe(SEMI);
            // if it doesnt read semi, read the condition of the for
            if(!see(SEMI))
                condition = expression();
            mustBe(SEMI);
            // if it doesnt read ')' <-- RPAREN read the updates of the for
            if(!see(RPAREN))
                updates= forUpdate();
            mustBe(RPAREN);
            body = statement();
            return new JForStatement(line, forInit,condition,updates,body);
        } else if(have(BREAK)){
            mustBe(SEMI);
            ...
            JBlock block = block();
            ArrayList<JFormalParameter> parameter = new ArrayList<JFormalParameter>();
            ArrayList<JBlock> blocks = new ArrayList<JBlock>();
            JBlock fBlock = null;
             while(see(CATCH)){
                mustBe(LPAREN);
                parameter.add(formalParameter());
                mustBe(RPAREN);
                blocks.add(block());
             }
             if (have(FINALLY))
                fBlock = block();
            return new JTryStatement(line, block, parameter, blocks, fBlock);
            } else if (have(THROW)){
    ...
    /* --------------------------- Helper functionsss --------------------------- */
        ...
        /*
        *  Special Function to look ahead for SEMI or COLON and comming back to the 
        * initial position after the look a head if complete.
        */
    ...
    private SwitchStatementGroup switchBlockStatementGroup() {
        ...
        while (see(CASE)) { // Default wasnt scanne.. meaning when the parse go through the case and find a default

                // The scanner will not read the default and the switch statement will be incompleted.
            labels.add(switchLabel());
        }
     ...
     *                                [ ( ASSIGN | PLUS_ASSIGN | DIV_ASSIGN | MINUS_ASSIGN | STAR_ASSIGN | REM_ASSIGN
     *                                    | ARSHIFT_ASSIGN | LRSHIFT_ASSIGN | ALSHIFT_ASSIGN | XOR_ASSIGN | OR_ASSIGN | AND_ASSIGN
     ...
     *
     * <pre>
     *   conditionalExpression ::= ConditionalOrExpression { QUESTION conditionalOrExpression }
     * </pre>
     *
    ...
    private JExpression conditionalExpression() {
        ...
        while (more) {
            if (have(QUESTION)) { /// Conditional expression mush have a colan. colon shouldnt be separated by itselft.
               JExpression thenPart, elsePart;
               thenPart = expression();
               mustBe(COLON);
               elsePart = expression();
    ...
    private JExpression inclusiveOrExpression() {
      ...
      /**
     * Parses a exclusive-and expression and returns an AST for it.
     *
     * <pre>
     *   exclusiveAndExpression ::= andExpression { XOR andExpression }
     * </pre>
     *
     * @return an AST for a exclusive-XOR expression.
     */
     ...
     *
     * <pre>
     *   equalityExpression ::= relationalExpression { (EQUAL | LNOT_ASSIGN) relationalExpression }
     * </pre>
     *
     ...
     *
     * <pre>
     *   relationalExpression ::= shiftExpression [ ( GT | GE | LE | LT ) shiftExpression
     ...
     *
     * <pre>
     *   shiftExpression ::= additiveExpression [ ( ARSHIFT | LRSHIFT | ALSHIFT ) additiveExpression
     *                                               | INSTANCEOF referenceType ]
     * </pre>
     *
     * @return an AST for a relational expression.
