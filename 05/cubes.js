function cubes_creation(r1, r2) {
	console.log("loading assignment 5...");
	console.log('r1',r1);
	console.log('r2',r2);

	let xtk = init_XTK(r1);
	let threejs = init_ThreeJS(r2);
	...
	var controller = {
		...
		'add new': () => {
			add_Cube_XTK(xtk);
			add_Cube_Three_JS(threejs);
		}
	};

	var gui = new dat.GUI();

	add_Dat_Gui_XTK(gui, xtk.cubes[0]);
	add_Dat_Gui_ThreeJS(gui, controller, threejs.cubes[0]);
	add_Dat_Gui_Both(gui, controller);
}

function init_XTK(container) {
...
function add_Cube_XTK(obj){
	var c = new X.cube();
	//c.center = [Math.random()*50, Math.random()*50, Math.random()*50];
	c.center = [50, 50, 50];
...
function init_ThreeJS(container, gui, controller) {
...
function add_Dat_Gui_XTK(gui, object) {
...
function add_Dat_Gui_ThreeJS(gui, controller, object) {
...
function add_Dat_Gui_Both(gui, controller) {
